...
crear_copia_seguridad() {
    ...
    if [[ ! -d "$BACKUP_DIR" ]]; then
        mkdir -p "$BACKUP_DIR"
        echo -e "${VERDE}Directorio de backup creado en $BACKUP_DIR.${NC}"
    fi

    # Limitar el número máximo de backups a 5
    backups=("${BACKUP_DIR}/sources.list.bak_"*)
    if [[ ${#backups[@]} -ge 5 ]]; then
        echo -e "${AZUL}El número máximo de backups ha sido alcanzado. Elimina algunos backups para hacer nuevos.${NC}"
        return 1
    fi

    archivo_backup="${BACKUP_DIR}/sources.list.bak_$(date +%Y%m%d_%H%M%S)"
    sudo cp "/etc/apt/sources.list" "$archivo_backup"
    echo -e "${VERDE}Copia de seguridad creada en $archivo_backup.${NC}"
}

# Función para restaurar una copia anterior del archivo
restaurar_copia_seguridad() {
    echo -e "${AZUL}=================================================${NC}"
    echo -e "${AZUL}| Restaurando una copia anterior de sources.list |${NC}"
    echo -e "${AZUL}=================================================${NC}"
    backups=("${BACKUP_DIR}/sources.list.bak_"*)
    ...
    read -p "$(echo -e ${AZUL}Ingresa el número de la copia de seguridad:${NC} )" numero_copia

    # Verificar si el número de copia de seguridad es válido
    if ! [[ "$numero_copia" =~ ^[1-5]$ ]]; then
        echo -e "${ROJO}Opción inválida.${NC}"
        return 1
    fi

    archivo_copia="${backups[$((numero_copia-1))]}"
    if [[ -f "$archivo_copia" ]]; then
        ...
        case $respuesta in
            [sS])
                sudo cp "$archivo_copia" "/etc/apt/sources.list"
...
modificar_sources_list() {
    ...
    echo -e "${AZUL}=================================================${NC}"
    echo -e "${AMARILLO}Agrega las líneas necesarias al archivo sources.list.${NC}"
...
agregar_opciones_msi() {
    ...
    echo -e "${AZUL}=================================================${NC}"
    agregar_a_archivo_si_no_existe "/etc/modprobe.d/snd-hda-intel.conf" "options snd-hda-intel enable_msi=1"
...
agregar_a_archivo_si_no_existe() {
    local archivo="$1"
    local entrada="$2"
...
configurar_passthrough_gpu() {
    ...
    agregar_a_archivo_si_no_existe "/etc/modprobe.d/blacklist.conf" "blacklist nouveau"
    agregar_a_archivo_si_no_existe "/etc/modprobe.d/blacklist.conf" "blacklist nvidia"

    # Agregar opciones de VFIO e IOMMU
    agregar_a_archivo_si_no_existe "/etc/modprobe.d/vfio.conf" "options vfio-pci ids=$GPU_VENDOR_ID disable_vga=1"
        ...
        case $opcion in
            1)
                while true; do
                    ...
                    echo -e "${AZUL} 2) Restaurar una copia anterior de sources.list${NC}"
